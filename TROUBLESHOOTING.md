...
| Calibration is not accepted | Retry the Smooth Pursuit sweep while keeping both eyes visible and following the target. MEYES requires all nine regions, horizontal/vertical following correlation, and a stable mapper fit; an explicitly configured stricter policy can also reject the metrics. |
...
| Live Input will not arm | After a passed calibration, confirm **Activate Live Input** in the dialog (or use the Live Input page). Verify Windows, a running camera, successful emergency-hotkey registration, and that physical mouse buttons plus Ctrl/Alt/Shift/Windows keys are released. Another application may own the emergency chord. A failed preflight must remain closed. |
