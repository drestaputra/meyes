...
Phase 0 through Phase 3 are complete, the bounded Phase 4 workflows are implemented, and Phase 5 now includes fail-closed normalized gaze features plus a distraction-free primary-display nine-point collection flow. Diagnostics shows the binocular eye-relative feature and its explicit availability state, but it is not a screen coordinate and cannot move the pointer. Calibration places its target at normalized 10/50/90% screen positions, supports Space/Enter/R/Escape controls, and retains ordered targets, explicit capture, quotas, attempt caps, replay/quality guards, per-target median/MAD outlier rejection, progress, and retry. Escape, presentation close, navigation away, tracking loss, Live Input arming, or shutdown cancels and erases volatile collection. Completing all targets invokes the replaceable quadratic mapper, fails closed on unsafe geometry, and displays deterministic per-target holdout RMSE, mean, maximum, and sample-count metrics. An optional all-or-none acceptance policy can evaluate explicitly configured evidence limits; because no representative physical-device benchmark exists yet, all limits default to unset and the UI truthfully reports `Review Required`. Only a policy-approved result is exposed through the accepted-result contract, and even that mapper remains volatile, unpersisted, inactive, and disconnected from pointer output. A configurable dormant One Euro point filter reduces small normalized-coordinate jitter and reseeds after stale gaps. A separate dormant primary-screen mapper clamps normalized predictions into validated inclusive physical-pixel bounds and reports clamping per axis. Neither component has an application runtime or executor connection. Physical-device evidence collection, mapper persistence, native screen-geometry acquisition, and gaze pointer output remain pending.
