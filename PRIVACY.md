...
| Calibration acceptance limits | Smooth Pursuit completion accepts a mapper in memory only after coverage, following-correlation, and stable-fit guards pass. Optional numeric limits are local configuration values for stricter rejection and persistence; persistence recovery remains disabled without a complete policy. |
